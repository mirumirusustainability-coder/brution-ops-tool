...
export function NotesTab({ company, onUpdate, projects = [], onUpdateProjectNotes }: NotesTabProps) {
  ...
  // Inline edit state for contact history
  const [editingIndex, setEditingIndex] = useState<number | null>(null);
  const [editDate, setEditDate] = useState(getToday());
  const [editContent, setEditContent] = useState('');
  const [editAuthor, setEditAuthor] = useState('');

  // Delete state for contact history
  const [deleteTargetIndex, setDeleteTargetIndex] = useState<number | null>(null);
  const [deleteConfirmText, setDeleteConfirmText] = useState('');

  // Inline edit state for project notes
  const [projectEditTarget, setProjectEditTarget] = useState<{ projectId: string; index: number } | null>(null);
  ...
  // Delete state for project notes
  const [projectDeleteTarget, setProjectDeleteTarget] = useState<{ projectId: string; index: number } | null>(null);
  ...
  const openInlineEdit = (index: number) => {
  ...
  const handleEditHistory = async () => {
    ...
    const nextHistory = contactHistory.map((entry, idx) =>
      idx === editingIndex
        ? { ...entry, date: editDate || getToday(), content: editContent.trim(), author: editAuthor.trim() || null }
        : entry
    );
    const nextDraft = { ...draft, contact_history: nextHistory };
    setDraft(nextDraft);
  ...
  const openProjectInlineEdit = (projectId: string, index: number) => {
  ...
  return (
    <div className="space-y-6">
        ...
        {sortedHistory.length === 0 ? (
          ...
          <div className="space-y-3">
            {sortedHistory.map(({ entry, index }) =>
              editingIndex === index ? (
                <div key={`${entry.date}-${index}`} className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm min-h-[80px] bg-white"
                    />
                  </div>
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={closeInlineEdit} className="px-3 py-1.5 text-sm border border-gray-300 rounded-md hover:bg-gray-50 bg-white">
                      취소
                    </button>
                    <button type="button" onClick={handleEditHistory} className="px-3 py-1.5 text-sm bg-primary text-white rounded-md hover:bg-primary-hover">
                      저장
                    </button>
                  </div>
                </div>
              ) : (
                <div key={`${entry.date}-${index}`} className="group flex items-start justify-between gap-3 rounded-lg border border-gray-100 p-3">
                  <div>
                  ...
                  <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      type="button"
                      onClick={() => { setDeleteTargetIndex(index); setDeleteConfirmText(''); }}
          ...
          <div className="space-y-3">
            {projectNoteItems.map((item) => {
              const isEditingThisNote =
                projectEditTarget?.projectId === item.projectId && projectEditTarget?.index === item.index;

              return isEditingThisNote ? (
                <div key={`${item.projectId}-${item.index}`} className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
                  <div className="grid gap-3 md:grid-cols-2">
                    <div>
                      <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">날짜</label>
                      <input
                  ...
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={closeProjectInlineEdit} className="px-3 py-1.5 text-sm border border-gray-300 rounded-md hover:bg-gray-50 bg-white">
                      취소
                    </button>
                    <button type="button" onClick={handleProjectEditSave} className="px-3 py-1.5 text-sm bg-primary text-white rounded-md hover:bg-primary-hover">
                      저장
                    </button>
                  </div>
                </div>
              ) : (
                <div key={`${item.projectId}-${item.index}`} className="group flex items-start justify-between gap-3 rounded-lg border border-gray-100 p-3">
                  <div>
                  ...
                  <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      type="button"
                      onClick={() => openProjectInlineEdit(item.projectId, item.index)}
                      className="p-1.5 rounded hover:bg-gray-100"
                      title="수정"
                    >
                      ✏️
                    ...
                    <button
                      type="button"
                      onClick={() => { setProjectDeleteTarget({ projectId: item.projectId, index: item.index }); setProjectDeleteConfirmText(''); }}
                      className="p-1.5 rounded hover:bg-red-50"
                      title="삭제"
                    >
                      🗑️
      ...
      {isEditing && (
        <div className="flex justify-end gap-2">
          <button type="button" onClick={handleCancel} className="px-4 py-2 text-sm border border-gray-300 rounded-md hover:bg-gray-50">
            취소
          </button>
          <button type="button" onClick={handleSave} disabled={saving} className="px-4 py-2 text-sm bg-primary text-white rounded-md hover:bg-primary-hover disabled:opacity-50">
            {saving ? '저장 중...' : '저장'}
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-sm rounded-lg p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => { setDeleteTargetIndex(null); setDeleteConfirmText(''); }}
      ...
      {projectDeleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-sm rounded-lg p-6 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => { setProjectDeleteTarget(null); setProjectDeleteConfirmText(''); }}
                className="px-4 py-2 border border-gray-300 rounded-md text-sm"
              >
                취소
              </button>
